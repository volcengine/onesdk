...
int chat_main() {
    ...
    if (result) {
        fprintf(stderr, "chat request failed with code3 %d\n", result);
        return result;
    }
    ...
    result = onesdk_chat(onesdk_ctx, &request_vision, output, &output_len);

    if (result) {
        fprintf(stderr, "chat request failed with code4 %d\n", result);
        return result;
    }
    result = onesdk_chat_wait(onesdk_ctx);
