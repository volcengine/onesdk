...
#if defined (CONFIG_AUDIO_SUPPORT_WAV_DECODER)
#define SAMPLE_RATE         16000
#endif
#define CHANNEL             1
...
audio_element_handle_t create_record_encoder_stream(void)
{
    ...
    ESP_LOGI(TAG, "Create wav encoder");
    wav_encoder_cfg_t wav_cfg = DEFAULT_WAV_ENCODER_CONFIG();
    wav_cfg.sample_rate = SAMPLE_RATE;
    wav_cfg.bits_per_sample = 16;
    wav_cfg.channel = CHANNEL;
    encoder_stream = wav_encoder_init(&wav_cfg);
#endif
...
audio_element_handle_t create_player_decoder_stream(void)
{
    ...
    decoder_stream = create_player_wav_decoder_stream();
#elif CONFIG_AUDIO_SUPPORT_RAW_DECODER
    ESP_LOGI(TAG, "Create raw decoder = NULL");
