...
int main() {
    iot_basic_config_t config = {
        .http_host = "iot-cn-shanghai.iot.volces.com",
    ...
    http_request_context_t *client = NULL;

    client = device_auth_client(client, &ctx);
    if (client == NULL) {
        printf("device auth client failed\n");
        goto done;
    }
    printf("device auth client success\n");
